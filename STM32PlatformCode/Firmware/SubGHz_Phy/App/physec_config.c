...
bool
check_quant_method(uint8_t qtype)
{
	switch (qtype) {
...
HAL_StatusTypeDef physec_config_write_eeprom(uint8_t offset_address, physec_config *config)
{
  ...
  buffer[offset++] = config->keygen.keygen_id;
	...
	// telemetry_config
	buffer[offset++] = *((uint8_t*)&config->telemetry); // 1 octet (bitfields packés)
	...
	while(buf_size-- && status == HAL_OK){
		status = HAL_FLASHEx_DATAEEPROM_Program(TYPEPROGRAMDATA_BYTE, eeprom_address, buffer[offset]);
		offset++ ;
		eeprom_address++ ;
	}
...
bool physec_config_read_eeprom(uint8_t offset_address, physec_config *config)
{
  ...
  config->keygen.keygen_id = buffer[offset++];
	...
	// telemetry_config
	*((uint8_t*)&config->telemetry) = buffer[offset++]; // 1 octet (bitfields packés)
